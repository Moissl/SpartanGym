...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['product_image']) && $_FILES['product_image']['error'] === UPLOAD_ERR_OK) {
        ...
        $target_dir = __DIR__ . "/../../../../assets/img/packageimg/";
        if (!is_dir($target_dir)) {
            mkdir($target_dir, 0777, true);
        }
        $target_file = $target_dir . $barcode . ".png";
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <button type="button" class="btn btn-info" data-toggle="modal" data-target="#barcodeModal">
                                <i class="bi bi-camera"></i> <?php echo $translations["scan"] ?? 'Escanear'; ?>
                            </button>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label for="product_image" class="form-label"><?php echo $translations["image"] ?? 'Imagen'; ?>:</label>
        ...
        <div class="modal-dialog modal-lg" role="document" style="max-width: 90%;">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="barcodeModalLabel"><?php echo $translations["product-barcode"]; ?> - <?php echo $translations["scan"] ?? 'Escanear'; ?></h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div id="scanner" style="width:100%; height:400px; border: 2px solid #ccc; overflow: hidden; position: relative;"></div>
                    <p class="text-center mt-2"><small><?php echo $translations["point-camera-barcode"] ?? 'Apunta la cámara al código de barras'; ?></small></p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal"><?php echo $translations["close"] ?? 'Cerrar'; ?></button>
                </div>
            </div>
        </div>
    </div>
