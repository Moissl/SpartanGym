...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['smtp_host'])) {
        ...
        $access_alert_enabled = $_POST['access_alert_enabled'] ?? 'FALSE';
        $access_alert_email = $_POST['access_alert_email'] ?? '';
        ...
        $env_data['ACCESS_ALERT_ENABLED'] = $access_alert_enabled;
        $env_data['ACCESS_ALERT_EMAIL'] = $access_alert_email;
        $env_data['MAIL_FROM_ADDRESS'] = $mail_from_address;
        $env_data['MAIL_FROM_NAME'] = $mail_from_name;
        ...
        try {
            ...
            // No recargar para permitir leer el error
        }
    }
}
                                ...
                                ?>
                                    <form method="POST">
                                        ...
                                        <div class="form-row">
                                            ...
                                            <div class="form-group col-md-6">
                                                <label for="mail_from_address">Correo del Remitente (Opcional):</label>
                                                <input type="email" class="form-control" id="mail_from_address" name="mail_from_address" 
                                                    value="<?= htmlspecialchars($env_data['MAIL_FROM_ADDRESS'] ?? '') ?>" placeholder="Ej: no-reply@tugimnasio.com">
                                                <small class="text-muted">Si se deja vacío, se usará el usuario SMTP.</small>
                                            </div>
                                            <div class="form-group col-md-6">
                                                <label for="mail_from_name">Nombre del Remitente (Opcional):</label>
                                                <input type="text" class="form-control" id="mail_from_name" name="mail_from_name" 
                                                    value="<?= htmlspecialchars($env_data['MAIL_FROM_NAME'] ?? '') ?>" placeholder="Ej: Gym One Soporte">
                                                <small class="text-muted">Si se deja vacío, se usará el nombre del negocio.</small>
                                            </div>

                                            <div class="form-group col-md-12">
                                                <hr>
                                                <h4>Alertas de Acceso</h4>
                                                <p class="text-muted">Enviar un correo al administrador cuando alguien intente entrar sin un pase válido.</p>
                                            </div>
                                            <div class="form-group col-md-3">
                                                <label for="access_alert_enabled">Activar Alertas</label>
                                                <div class="custom-control custom-switch">
                                                    <input type="hidden" name="access_alert_enabled" value="FALSE">
                                                    <input type="checkbox" class="custom-control-input" id="access_alert_enabled" name="access_alert_enabled" value="TRUE" <?= (htmlspecialchars($env_data['ACCESS_ALERT_ENABLED'] ?? '') === 'TRUE') ? 'checked' : '' ?>>
                                                    <label class="custom-control-label" for="access_alert_enabled">Sí, enviar alertas</label>
                                                </div>
                                            </div>
                                            <div class="form-group col-md-9">
                                                <label for="access_alert_email">Correos para recibir alertas (separados por coma):</label>
                                                <input type="text" class="form-control" id="access_alert_email" name="access_alert_email" value="<?= htmlspecialchars($env_data['ACCESS_ALERT_EMAIL'] ?? '') ?>" placeholder="admin@gym.com, gerente@gym.com">
                                            </div>
                                        </div>
                                        <button type="submit"
