...
// Lógica para Promociones (Galería)
$promo_dir = '../../../assets/img/promotions/';
if (!is_dir($promo_dir)) {
    mkdir($promo_dir, 0777, true);
}

if (isset($_POST['uploadPromotion']) && isset($_FILES['promotionFile'])) {
    $total_files = count($_FILES['promotionFile']['name']);
    $uploaded_count = 0;

    for ($i = 0; $i < $total_files; $i++) {
        $filename = $_FILES['promotionFile']['name'][$i];
        $tmp_name = $_FILES['promotionFile']['tmp_name'][$i];
        $size = $_FILES['promotionFile']['size'][$i];
        $error = $_FILES['promotionFile']['error'][$i];

        $file_type = strtolower(pathinfo($filename, PATHINFO_EXTENSION));
        $allowed_types = array('png', 'jpg', 'jpeg');

        if (in_array($file_type, $allowed_types) && $size <= 4000000 && $error === 0) {
            $new_filename = uniqid() . '_' . $i . '.' . $file_type;
            if (move_uploaded_file($tmp_name, $promo_dir . $new_filename)) {
                $uploaded_count++;
            }
        }
    }
    ...
    if ($uploaded_count > 0) {
        $alerts_html .= "<div class='alert alert-success'>Se han subido $uploaded_count imágenes exitosamente.</div>";
    } else {
        $alerts_html .= "<div class='alert alert-warning'>No se pudieron subir las imágenes. Verifique el formato y tamaño.</div>";
    }
}
...
if (isset($_POST['deletePromotion'])) {
    ...
    if (basename($fileToDelete) == $fileToDelete && file_exists($promo_dir . $fileToDelete)) {
        unlink($promo_dir . $fileToDelete);
        $alerts_html .= "<div class='alert alert-success'>Imagen eliminada exitosamente</div>";
        header("Refresh:1");
    }
}
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="card shadow" style="margin-top: 20px;">
                            <div class="card-header">
                                Promociones y Novedades (Visible en Index)
                            </div>
                            <div class="card-body">
                                <form action="" method="post" enctype="multipart/form-data" class="form-inline">
                                    <div class="form-group">
                                        <label for="promotionFile" style="margin-right: 10px;">Subir nueva imagen:</label>
                                        <input type="file" class="form-control-file" id="promotionFile" name="promotionFile[]" accept="image/png, image/jpeg, image/jpg" multiple required>
                                    </div>
                                    <button type="submit" class="btn btn-primary" name="uploadPromotion"><i class="bi bi-upload"></i> Subir</button>
                                </form>
                                <hr>
                                <div class="row">
                                    <?php
                                    $promo_files = scandir($promo_dir);
                                    foreach ($promo_files as $file) {
                                        if (in_array(strtolower(pathinfo($file, PATHINFO_EXTENSION)), ['jpg', 'jpeg', 'png'])) {
                                            echo '<div class="col-md-3 text-center" style="margin-bottom: 15px;">';
                                            echo '<div class="thumbnail" style="padding: 10px; border: 1px solid #ddd; border-radius: 4px;">';
                                            echo '<img src="' . $promo_dir . $file . '" style="height: 150px; object-fit: cover; width: 100%; margin-bottom: 10px;">';
                                            echo '<form method="post"><input type="hidden" name="delete_file" value="' . $file . '"><button type="submit" name="deletePromotion" class="btn btn-danger btn-xs btn-block"><i class="bi bi-trash"></i> Eliminar</button></form>';
                                            echo '</div></div>';
                                        }
                                    }
                                    ?>
