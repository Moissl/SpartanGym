...
// FIX: Asegurar que las columnas de fecha soporten hora (DATETIME)
$columns_to_fix = ['buydate', 'expiredate'];
foreach ($columns_to_fix as $col) {
    $check_res = $conn->query("SELECT DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '$db_name' AND TABLE_NAME = 'current_tickets' AND COLUMN_NAME = '$col'");
    if ($check_res && $check_res->num_rows > 0) {
        $row = $check_res->fetch_assoc();
        if (strtolower($row['DATA_TYPE']) === 'date') {
            $conn->query("ALTER TABLE current_tickets MODIFY $col DATETIME");
        }
    }
}
...
if (is_null($expire_day) || $expire_day == 0 || strtolower($expire_day) === 'ilimitado') {
    $expire_date = $translations["unlimited"];
    $expire_date_db = '9999-12-31 23:59:59';
} else {
    $currentDate = new DateTime();
    if ($expire_day == 1) {
        $currentDate->add(new DateInterval('PT12H'));
    } else {
        $currentDate->add(new DateInterval('P' . $expire_day . 'D'));
    }
    $expire_date = $currentDate->format('Y-m-d');
    $expire_date_db = $currentDate->format('Y-m-d H:i:s');
}

$partnerLogoSrc = "https://gymoneglobal.com/assets/img/logo.png";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Dividir el HTML en partes más pequeñas para evitar el límite de pcre.backtrack_limit
    $parts = str_split($invoiceHtml, 50000);
    ...
    // Determine DB payment method standard value
    $dbPaymentMethod = 'Cash';
    if ($paymentMethod == 'card') $dbPaymentMethod = 'Card';
    elseif ($paymentMethod == 'profile') $dbPaymentMethod = 'Balance';

    $status = "paid";
    $pathinvoicesql = "{$userid}-{$invoiceNumber}.pdf";

    // Insert invoice with type, payment_method and description (ticket name)
    $stmt = $conn->prepare("INSERT INTO invoices (userid, name, price, type, payment_method, status, route, created_at, description) VALUES (?, ?, ?, 'Ticket', ?, ?, ?, NOW(), ?)");
    $stmt->bind_param("isdssss", $userid, $fullname, $ticketprice, $dbPaymentMethod, $status, $pathinvoicesql, $ticketname);

    if ($stmt->execute()) {
        $alerts_html .= '<div class="alert alert-success" role="alert">'. $translations["invoiceadded"] .'</div>';
    } else {
    ...
    $stmt->bind_param("isssi", $userid, $ticketname, $datetime, $expire_date_db, $occasions);
...
EOD;
        ...
        if (isset($_GET['no_entry']) && $_GET['no_entry'] === 'true') {
            header("Location: ../../../dashboard/?status=paid");
        } else {
...
<body>
    ...
    <div id="loading-overlay">
        ...
        <h4 style="margin-top: 20px; color: #333;">Procesando transacción...</h4>
    </div>
    ...
    <script>
        $(document).ready(function() {
            $('#paymentModal form').on('submit', function() {
