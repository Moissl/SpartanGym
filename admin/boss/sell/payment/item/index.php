...
";
    ...
    foreach ($data as $rowItem) {
        $product_names[] = htmlspecialchars($rowItem['name']) . " (x" . intval($rowItem['quantity']) . ")";
    }
    $description = implode(', ', $product_names);

    // Map DB payment method
    ...
    $stmt = $conn->prepare("INSERT INTO invoices (userid, name, price, type, payment_method, status, route, created_at, description) VALUES (?, ?, ?, 'Product', ?, ?, ?, NOW(), ?)");
    $stmt->bind_param("isdssss", $userid, $fullname, $total, $dbPaymentMethod, $status, $pathinvoicesql, $description);
