...
function resizeImage($source, $destination, $max_width = 200, $max_height = 200) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        if ($image_extension === 'png') {
            ...
            if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {                resizeImage($target_file, $target_file, 200, 200);                $sql = "UPDATE trainers SET image='$target_file' WHERE id='$trainer_id'";
                if ($conn->query($sql) === TRUE) {
