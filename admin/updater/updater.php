...
function createBackup($root_dir)
{
    ...
    $zip_file = $backup_dir . DIRECTORY_SEPARATOR . "backup_pre_update_$timestamp.zip";

    logMessage("Creando copia de seguridad en: $zip_file");

    $zip = new ZipArchive();
    if ($zip->open($zip_file, ZipArchive::CREATE | ZipArchive::OVERWRITE) !== true) {
        logMessage("ADVERTENCIA: No se pudo crear el archivo de respaldo. Continuando con riesgo...");
        return;
    }
    ...
    foreach ($files as $name => $file) {
        if (!$file->isDir()) {
            ...
            $relativePath = substr($filePath, strlen(realpath($root_dir)) + 1);

            // Excluir la carpeta de backups, git y los archivos temporales del actualizador
            if (strpos($relativePath, 'backups') === 0 || strpos($relativePath, '.git') === 0 || strpos($filePath, __DIR__) !== false) {
                continue;
            }
    ...
    $zip->close();
    logMessage("Copia de seguridad completada exitosamente.");
}
